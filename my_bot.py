...
async def start(message: types.Message, state: FSMContext):
    ...
    if not cursor.fetchone():
        ...
        await message.answer("أهلاً بك مجدداً! اختر ما تريد:", reply_markup=main_menu())
...
async def set_name(message: types.Message, state: FSMContext):
    ...
    db.commit()
    await message.answer("تم الحفظ!", reply_markup=main_menu())
    await state.clear()
...
async def show_dashboard(call: types.CallbackQuery):
    ...
    await call.message.edit_text(dashboard, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="🔙 القائمة", callback_data="main_menu")]]))

@dp.callback_query(F.data == "add_balance")
async def add_balance(call: types.CallbackQuery, state: FSMContext):
    txt = "💰 نظام شحن الرصيد\n━━━━━━━━━━━━━━━\n\nيرجى التحويل لـ: **01011496150**\n(فودافون كاش / إنستا باي)\n\nبعد التحويل، أرسل المبلغ (أرقام فقط):"
    await call.message.edit_text(txt, reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="🔙 رجوع", callback_data="main_menu")]]))
    await state.set_state(UserStates.waiting_for_amount)

@dp.message(UserStates.waiting_for_amount)
async def get_amount(message: types.Message, state: FSMContext):
    await state.update_data(amount=message.text)
    await message.answer("✅ أرسل سكرين شوت التحويل الآن:")
...
async def handle_photo(message: types.Message, state: FSMContext):
    ...
    amt = data.get('amount')
    kb = types.InlineKeyboardMarkup(inline_keyboard=[[types.InlineKeyboardButton(text="✅ قبول", callback_data=f"app_{message.from_user.id}_{amt}"), types.InlineKeyboardButton(text="❌ رفض", callback_data="rej_")]])
    ...
    await message.answer("تم إرسال الطلب للمراجعة.")
    await state.clear()

@dp.callback_query(F.data == "categories")
async def show_categories(call: types.CallbackQuery):
    btns = [[types.InlineKeyboardButton(text=v['name'], callback_data=f"cat_{k}")] for k, v in CATEGORIES.items()]
    btns.append([types.InlineKeyboardButton(text="🔙 القائمة", callback_data="main_menu")])
...
async def get_qty(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(f"💰 الإجمالي: {total} ج.م\nأرسل الرابط:")
...
async def finish_order(msg: types.Message, state: FSMContext):
    ...
    if req.status_code == 200:
        ...
        db.commit()
        await msg.answer("✅ تم طلب الخدمة!")
    await state.clear()
